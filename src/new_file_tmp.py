...
def buyPizza(price, balance):
    ...
    # It then creates a dictionary with the price, type, and size of the pizza, which is returned.
     ...
     pizza = {
         "price": price,
@@ -39,8 +41,11 @@ def buyPizza(price, balance):
         "size": "huge"
     }
     
     # Check if there's enough balance to buy the pizza
     enough = not_enough_balance(new_balance, price)  # Check if there's enough balance
     if enough:
         # Return the pizza if there's enough balance
         return pizza
     else:
         # Return None if there's not enough balance
         return None
