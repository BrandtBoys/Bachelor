...
def test_run_exclude_patterns():
    ctx = run_command.make_context("run", ["--exclude-patterns", __file__])
    assert ctx.params["exclude_patterns"] == [__file__]
## PrunedCode: def test_cli_name(test_apps):
    """Make sure the CLI object's name is the app's name and not the app itself"""
    from cliapp.app import testapp

    assert testapp.cli.name == testapp.name, Comment: # This function tests that the CLI object's name matches the app's name, ensuring consistency in naming conventions.Code: def test_find_best_app(test_apps):
    class Module:
        app = Flask("appname")
    ...
    class Module:
        @staticmethod
        , Comment: # This function tests various scenarios to ensure that it correctly identifies and returns a Flask application object from a given test module.Code: def create_app():
            raise TypeError("bad bad factory!"), Comment: # This function raises an error indicating that the application creation process has failed due to a faulty factory.Code: def test_prepare_import(request, value, path, result):
    """Expect the correct path to be set and the correct import and app names
    to be returned.
    ...
    assert prepare_import(value) == result
    assert sys.path[0] == str(path), Comment: # This function tests the `prepare_import` function, ensuring it correctly sets the import path and returns the expected results.Code: def test_locate_app_raises(test_apps, iname, aname):
    with pytest.raises(NoAppException):
        locate_app(iname, aname), Comment: # This function tests that the 'locate_app' function raises a 'NoAppException' when given invalid application names.Code: def test_locate_app_suppress_raise(test_apps):
    app = locate_app("notanapp.py", None, raise_if_not_found=False)
    assert app is None
    ...
    with pytest.raises(NoAppException):
        locate_app("cliapp.importerrorapp", None, raise_if_not_found=False), Comment: # This function tests the behavior of `locate_app` when a non-existent app is searched and raises a `NoAppException`.Code: def test_get_version(test_apps, capsys):
    class MockCtx:
        resilient_parsing = False
        color = None

        , Comment: # This function tests the functionality of the get_version function by mocking its context and verifying that it correctly outputs the versions of Python, Flask, and Werkzeug.Code: def exit(self):
            return, Comment: # This function appears to be an empty method, possibly intended for cleanup or termination purposes.Code: def test_scriptinfo(test_apps, monkeypatch):
    obj = ScriptInfo(app_import_path="cliapp.app:testapp")
    app = obj.load_app()
    assert app.name == "testapp"
    assert obj.load_app() is app
    ...
    obj = ScriptInfo()
    app = obj.load_app()
    assert app.name == "testapp", Comment: # This function tests the functionality of the ScriptInfo class,
# specifically its ability to load and identify applications from different import paths.Code: def test_app_cli_has_app_context(app, runner):, Comment: # This function tests if the application context is available when running an app using the CLI.Code: def _param_cb(ctx, param, value):
      
        
        return bool(current_app), Comment: # This function checks if the `current_app` is available within a parameter callback and returns its boolean value.Code: def check(value):
        app = click.get_current_context().obj.load_app()
      
        
        same_app = current_app._get_current_object() is app
        return same_app, value, Comment: # Check if the current application matches the loaded application.